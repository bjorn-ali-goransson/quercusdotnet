...
public class QuercusParser {
  ...
  private Dictionary<string, int> _insensitiveReserved { get; } = new Dictionary<string, int>
  {
  ...
  private Dictionary<StringValue,StringValue> _namespaceUseMap
    = new Dictionary<StringValue,StringValue>();
  ...
  private List<String> _loopLabelList = new List<String>();
  private int _labelsCreated;

  private string _comment;

  public QuercusParser(QuercusContext quercus) : this(quercus, quercus != null ? quercus.getScriptEncoding() : "utf-8")
  {
  }
  ...
  {
    return _quercus != null && _quercus.getInibool("short_open_tag");
  }
    ...
    
  {
    ...
    List<Statement> stmtList = parseStatementList();
    ...
    
  {
    ...
    while (true) {
      ...
      case FINAL:
        {
          ...
          do {
            ...
            case CLASS:
              ...
              break;
            default:
              throw error(("expected 'class' at {0}",
                              tokenName(token)));
            }
          } while (token != CLASS);
        }
      ...
      case '{':
        {
          List<Statement> enclosedStatementList = parseStatementList();
    ...
    
  {
    ...
    default:
      ...
      /*
    default:
      throw error(("unexpected token {0}.", tokenName(token)));
      */
    }
  }
   ...
   */
  private void parseEcho(List<Statement> statements)
                                    ...
                                    Expr expr)
  {
    ...
    else if (expr is BinaryAppendExpr) {
    ...
    else if (expr is LiteralStringExpr) {
   ...
   * Parses the unset statement.
   */
  private void parseUnset(List<Statement> statementList)
    ...
    
  {
    ...
    try {
      ...
      List<BlockStatement> blockList = new List<BlockStatement>();

      List<Integer> fallThroughList = new List<Integer>();
      BlockStatement defaultBlock = null;

      while ((token = parseToken()) == CASE || token == DEFAULT) {
        Location caseLocation = getLocation();

        List<Expr> valueList = new List<Expr>();
        ...
        List<Statement> newBlockList = parseStatementList();
    ...
    
  {
    ...
    try {
      ...
      if (token == ARRAY_RIGHT) {
        if (isRef)
          throw error(("key reference is forbidden in foreach"));
      ...
      if (token != ')')
        throw error(("expected ')' in foreach"));
    ...
    
  {
    ...
    try {
      ...
      if (isAbstract && ! _scope.isAbstract()) {
        if (_classDef != null) {
          throw error(("'{0}' may not be abstract because class {1} is not abstract.",
                          nameV, _classDef.getName()));
        }
        else {
          throw error(("'{0}' may not be abstract. Abstract functions are only "
      ...
      if (_classDef != null
        ...
        if (isStatic) {
          throw error(("'{0}:{1}' may not be static because class constructors may not be static",
      ...
      if (_classDef != null
          ...
          && args.length != 2)
      {
        throw error(("{0}::{1} must have exactly two arguments defined",
    ...
    
  {
    ...
    try {
      ...
      Arg []useArgs;
      List<VarExpr> useVars = new List<VarExpr>();
  ...
  {
    LinkedDictionary<StringValue, Arg> argMap
      = new LinkedDictionary<StringValue, Arg>();
    ...
    while (true) {
      ...
      if (argMap.get(argName) != null && _quercus.isStrict()) {
        throw error(("aliasing of function argument '{0}'", argName));
      }
    ...
    
  {
    ...
    if (! _isTop && _loopLabelList.size() == 0 && ! _quercus.isLooseParse())
      throw error(("cannot 'break' inside a function"));
    ...
    case ';':
      ...
      return _factory.createBreak(location,
                                  null,
                                  (List<String>) _loopLabelList.clone());
  ...
  {
    if (! _isTop && _loopLabelList.size() == 0 && ! _quercus.isLooseParse())
      throw error(("cannot 'continue' inside a function"));
   ...
   */
  private List<Statement> parseConstDefinition()
    
  {
    List<Statement> constList = new List<Statement>();
    ...
    
  {
   ...
   List<StringValue> traitList = new List<StringValue>();
    ...
    if (token == '{') {
      while ((token = parseToken()) >= 0 && token != '}') {
        ...
        if (token == SCOPE) {
          ...
          throw error(("cannot resolve method because multiple traits are defined"));
        }
        ...
        if (token == INSTEADOF) {
          ...
          throw error(("expected 'insteadof' or 'as' at {0}",
                          tokenName(token)));
        }
  ...
  private List<Statement> parseNamespace()
    
  {
    ...
    if (token == '{') {
      ...
      throw error(("namespace must be followed by '{' or ';'"));
    }
  }
    ...
    
  {
    ...
    case is:
      ...
      if (classNameExpr is ConstExpr) {
        ...
        return _factory.createis(expr, className);
      }
      else {
        return _factory.createisVar(expr, classNameExpr);
      }
    ...
    
  {
    ...
    case TRUE:
      return _factory.createLiteral(boolValue.TRUE);

    case FALSE:
      return _factory.createLiteral(boolValue.FALSE);
    ...
    case STATIC:
      {
        if (_classDef == null) {
          throw error(("cannot use new {0}() outside of a class context.",
                          tokenName(token)));
        }
    ...
    case '(':
      {
        ...
        if (expr is ConstExpr) {
          ...
          if ("bool".equalsIgnoreCase(type)
            ...
            return _factory.createTobool(parseAssignExpr());
    ...
    default:
      throw error(("{0} is an unexpected token, expected an expression.",
                      tokenName(token)));
    }
  }
    ...
    
  {
    ...
    if (token == '$')
      ...
      throw error(("expected variable at {0} as left-hand-side",
                      tokenName(token)));
    ...
    
  {
    ...
    throw error(("unexpected token '{0}' in class scope expression",
                    tokenName(token)));
  }
    ...
    
  {
    ...
    if (token == THIS) {
      ...
      throw error(("Expected identifier at '{0}'", tokenName(token)));

    if (_lexeme.indexOf('\\') >= 0) {
      throw error(("Namespace is not allowed for variable ${0}", _lexeme));
    }
    ...
    
  {
    ...
    /*
     if (name.equals("each")) {
        if (args.size() != 1)
          throw error(("each requires a single expression"));
  ...
  {
    if (_classDef == null)
      throw error(("'self' is not valid because there is no active class."));
    ...
    
  {
    ...
    // XX: unicode issues?
    if (nameExpr.isLiteral() || nameExpr is ConstExpr) {
    ...
    
  {
    ...
    List<Expr> leftVars = new List<Expr>();
    ...
    
  {
    ...
    if (token == '(') {
      ...
      if (args.size() > 0)
        ...
        throw error(("empty must have one arg"));
    }
    else {
      throw error(("expected '('"));
    }
    ...
    
  {
    ...
    if (token != beginChar)
      throw error(("Expected {0}", String.valueOf(beginChar)));
    ...
    
  {
    ...
    while (true) {
      ...
      if (token == IDENTIFIER) {
        ...
        throw error(("'{0}' is an unexpected token in import",
                        tokenName(token)));
      }
    }
      ...
      case '<':
        ...
        if (ch == '<') {
          ...
          if (! parseTextMatch(sb, "script"))
            throw error(("expected 'script' at '{0}'", sb));
    ...
    
  {
    ...
    if (_lexeme.indexOf('\\') >= 0) {
      throw error(("namespace identifier is not allowed at '{0}'",
                      _lexeme));
    }
    else if (_peek == '\\') {
      throw error(("namespace identifier is not allowed at '{0}\\'",
    ...
    
  {
    ...
    if (ch != '\n') {
      throw expect(("nowdoc newline"), ch);
    }
    ...
    
  {
    ...
    if (ch == '$') {
      ...
      throw error(("Unexpected character at {0}",
                      String.valueOf((char) ch)));

    if (ch != ']')
      throw error(("Expected ']' at {0}",
                      String.valueOf((char) ch)));
  ...
  {
    if (value is StringBuilderValue) {
    ...
    
  {
    ...
    while ((ch = read()) > 0) {
      ...
      /* quercus/013c
        ...
        throw error(("unexpected newline in string."));
      */
      else {
        _sb.append((char) ch);
  ...
  {
    return error(("expected {0} at {1}", expected, tokenName(token)));
  }
  ...
  private class ParserLocation {
    ...
    private override bool Equals(String s1, string s2)
    {
